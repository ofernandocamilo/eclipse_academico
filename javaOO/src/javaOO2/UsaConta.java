...
public class UsaConta {
	public static void main(String[] args) { //classe principal, controladora;
		...
		//Uma variável foi criada (ValorSaque) passando o valor do saque que será de 3500.
		//Abaixo uma outra variável do tipo booleano atribuída ao método  c1.sacar(ValorSaque);
		//No If baixo é criada as condições segundo os parâmetros de true/false do boolean;

		double ValorSaque = 3500;
		boolean resultado = c1.sacar(ValorSaque);

		if(resultado) {
			System.out.println("Saldo atualizado após sacar 100 reais do Robert: "+c1.saldo); //1ª parte true do boolean
		} else {
			System.out.println("Não posso sacar um valor maior que o saldo!"); //2ª parte false do boolean
		}
